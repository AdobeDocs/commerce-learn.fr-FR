---
title: Utilisation des attributs de produit avec PWA Studio
description: Découvrez comment gérer les attributs de produit avec PWA Studio.
doc-type: feature video
duration: 463
audience: all
activity: use
kt: 13789
last-substantial-update: 2023-04-27T00:00:00Z
feature: Configuration, System
topic: Commerce, Administration
role: Admin, User
level: Beginner, Intermediate
exl-id: 71210a1b-620b-4bb7-aa96-4afd01cff7d9
source-git-commit: 9aa4d70ee6a3825f027aa2a9c6a1ac0f876ed59f
workflow-type: tm+mt
source-wordcount: '63'
ht-degree: 0%

---

# Utilisation des attributs de produit avec PWA Studio

Dans cette vidéo, découvrez comment gérer les attributs de produit lorsque vous disposez d’un storefront créé avec PWA Studio.

## À qui s&#39;adresse cette vidéo ?

* Développeurs front-end
* Chefs de projet

## Contenu vidéo

>[!VIDEO](https://video.tv.adobe.com/v/343788?learn=on)

## Ressources supplémentaires

* [Guide de gestion des catalogues - Attributs  [!DNL Commerce]  produit](https://experienceleague.adobe.com/docs/commerce-admin/catalog/product-attributes/product-attributes.html?lang=fr)
* [PWA Studio - Documentation pour les développeurs](https://developer.adobe.com/commerce/pwa-studio/)
