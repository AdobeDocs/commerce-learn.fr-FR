...
Ce modèle d’architecture de référence globale est basé sur le compositeur et est conçu pour tirer le meilleur parti de toutes les fonctionnalités du compositeur.
...
* Réutilisation du code par le biais de référentiels de code partagés
* Flexibilité totale lors de l&#39;installation des packages, chaque package GRA peut être mis à niveau, rétrogradé ou rétroporté individuellement
...
Le fragment de code ci-dessus est le fichier composer.json d’un métapaquet. Les métapaquets contiennent uniquement un fichier composer.json et aucun autre code. Le code ci-dessus est également le métapaquet complet. Hébergez-le dans un référentiel Git et vous disposez d’un référentiel du compositeur de métapaquets installable. Un exemple de module GRA et un module tiers, ainsi que le module principal Adobe Commerce sont nécessaires. Il nécessite également la technologie gra-component-foundation, que le chapitre suivant explique.
...
Le développement se produit dans le répertoire du fournisseur. Demandez au compositeur d’installer vos packages de base à partir de la source. Ce faisant, les packages sont extraits à partir de Git au lieu d’être installés à partir d’une archive téléchargée.
...
Le problème de lenteur se produit lorsque vous disposez de plusieurs référentiels VCS dans composer.json. Chaque référentiel de compositeur doit être lu lors des mises à niveau. En outre, disposer de 50 référentiels pour 50 packages représente une surcharge 50 fois supérieure à celle d’un seul référentiel de compositeur.
...
Appliquez le contrôle de version sémantique. Vous pouvez tout savoir sur le contrôle de version sémantique à l’adresse <https://semver.org/>. En particulier, la FAQ est une lecture recommandée. Avec le contrôle de version sémantique, les nombres dans « 1.0.0 » sont appelés MAJEUR.MINEUR.PATCH. Les versions mineures et de correctif d’un package peuvent être introduites en toute sécurité sans interrompre l’application.Vous pouvez inclure automatiquement des correctifs et choisir manuellement des mises à niveau mineures. Gardez à l’esprit que cela entraîne des frais généraux supplémentaires en sélectionnant manuellement chaque modification mineure :
...
Créez uniquement des branches de fonctionnalités dans vos référentiels de packages. Pas dans les référentiels d’installation de votre boutique. Restez en mesure d’apporter des modifications à votre boutique à l’aide du compositeur. Évitez d’avoir à effectuer des fusions Git sur le référentiel de déploiement.

Types de branches courants dans les stratégies d’embranchement et référentiels dans lesquels ils existent :
...
Les exemples de code de cet article ont été combinés dans un ensemble de référentiels Git que vous pouvez utiliser pour explorer la preuve de concept.
